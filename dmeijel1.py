...
def andmebaas():
    conn = sqlite3.connect ("dmeijel.db")
    c = conn.cursor()
    ...
    c.execute('INSERT INTO dmeijel (first_name, last_name, email, car_make, car_model, car_year, car_price) VALUES (?, ?, ?, ?, ?, ?, ?)', (first_name, last_name, email, car_make, car_model, car_year, car_price))
    conn.commit()
    
    conn.close()

def andmed():
    conn = sqlite3.connect('epood_dmeijel.db')
    c = conn.cursor()
    c.execute("SELECT * FROM dmeijel")
    andmed1 = c.fetchall()
    for rida in andmed1:
        print(rida)
    conn.close()
...
def kustutamine():
    conn =sqlite3.connect('epood_dmeijel.db')
    inp = conn.cursor()
    k = input("Sisesta ID mille soovid kustutada: ")
    inp.execute("DELETE FROM dmeijel WHERE id = ?", (k,))
    conn.commit()
    conn.close()
...
def keskmine_kallim():
    ...
    c.execute("SELECT AVG(car_year) FROM dmeijel")
    avg_aasta = c.fetchone()[0]
    print("Keskmine autode aasta:", avg_aasta)

    c.execute("SELECT MAX(car_price) FROM dmeijel")
...
def vanemad_autod():
    conn = sqlite3.connect('epood_dmeijel.db')
    c = conn.cursor()
    sql = "SELECT * FROM dmeijel WHERE car_year < 2000 ORDER BY car_year ASC"
    c.execute(sql)
    results = c.fetchall()
    for row in results:
        print(row)
...
def automark():
    conn = sqlite3.connect('epood_dmeijel.db')
    c = conn.cursor()
    sql = "SELECT car_make, car_model FROM dmeijel ORDER BY car_year DESC LIMIT 5"
    c.execute(sql)
    results = c.fetchall()
